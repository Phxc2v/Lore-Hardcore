...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- 2026-05-05: пользователь попросил max сумма ~90 troops в орочьей бандитской партии. -->
    <xsl:template match="MBPartyTemplate[@id='greenskin_bandit_template']/stacks/PartyTemplateStack[@troop='NPCCharacter.tor_gs_bandit_raider']/@max_value">
        <xsl:attribute name="max_value">60</xsl:attribute>
    </xsl:template>
    ...
    <xsl:template match="MBPartyTemplate[@id='greenskin_bandit_template']/stacks/PartyTemplateStack[@troop='NPCCharacter.tor_gs_bandit_raider_arrer']/@max_value">
        <xsl:attribute name="max_value">20</xsl:attribute>
    </xsl:template>
    ...
    -->
    <!-- 2026-05-21: ОТКЛЮЧЕНО по фидбеку клиента «в деревнях орков не пацаны а обычные орки».
         Возвращаем vanilla TOR orc_boy в militia (пацаны-воины, как лор и предполагает).
         Раньше меняли на tor_gs_orc и поднимали max_value — это противоречило TOR design.
         XSLT-override применяется всегда (даже при master toggle OFF), поэтому решили
         просто убрать. Балансовая разница: militia деревень = vanilla TOR (8 пацанов max).
         (Файл оставлен закомментированным на случай, если потребуется вернуть.)
    -->
    <!--
