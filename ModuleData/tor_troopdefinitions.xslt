...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!-- Тролль — лорная пропорция «1 тролль = 6-8 человек».
         v0.1.4.68: Endurance 800 → 500 (срез ~37%). С HP×1.40 в реальном бою
         даёт ~700 HP (≈9× человека), в AI-бою с HP×1.20 ~600 HP (≈8× человека).
         Skill +30% не применяется (отдельный exclude в OrcSkillValuePatch),
         Athletics ×0.85 (TrollAthleticsNerfPatch). Лорно: «огромная мясная гора,
         медленная, но мощная — нужно 6-8 копейщиков, чтобы свалить». -->
    ...
    <!-- ============ Chaos троп-деревья (по рисункам клиента, Picc) ============ -->
    <!-- ДЕРЕВО КУЛЬТИСТОВ (photo_33): удлиняем ветку — Послушник Хаоса (acolyte_of_chaos, T3)
         теперь апается в Культист-магистр Хаоса (cult_magister, T4). Раньше acolyte был концом.
         acolyte уже носит equipment cult_magister_template → апгрейд логичен. -->
    <xsl:template match="NPCCharacter[@id='tor_chaos_acolyte_of_chaos']/upgrade_targets">
        <upgrade_targets>
            <upgrade_target id="NPCCharacter.tor_chaos_cult_magister" />
        </upgrade_targets>
    </xsl:template>

    <!-- ДЕРЕВО НОРСКИ (photo_32). Соплеменник→{Разбойник,Китобой,Всадник} уже есть.
         Достраиваем 3-веточную схему: новые узлы Дикарь/Воин/Метатель/Чемпион-Метатель/Наездник
         в lh_chaos_troops.xml; здесь перенаправляем существующих родителей. -->
    <!-- Разбойник(raider) → Мародёр + Дикарь (раздвоение ветки) -->
    <xsl:template match="NPCCharacter[@id='tor_chaos_norscan_raider']/upgrade_targets">
        <upgrade_targets>
            <upgrade_target id="NPCCharacter.tor_chaos_norscan_marauder" />
            <upgrade_target id="NPCCharacter.tor_lh_norscan_savage" />
        </upgrade_targets>
    </xsl:template>
    <!-- Охотник(reaver) → Метатель -->
    <xsl:template match="NPCCharacter[@id='tor_chaos_norscan_reaver']/upgrade_targets">
        <upgrade_targets>
            <upgrade_target id="NPCCharacter.tor_lh_norscan_thrower" />
        </upgrade_targets>
    </xsl:template>
    <!-- Конюший(horsemaster) → Наездник -->
    <xsl:template match="NPCCharacter[@id='tor_chaos_norscan_horsemaster']/upgrade_targets">
        <upgrade_targets>
            <upgrade_target id="NPCCharacter.tor_lh_norscan_rider" />
        </upgrade_targets>
    </xsl:template>
</xsl:stylesheet>
